  ...
  const renderSkills = (category) => {
    return skills
      ...
      .map((skill, index) => {
        ...
        return (
          <div key={index} className="flex max-md:flex-col max-md:items-center gap-4">
            <IconComponent className="text-primary text-xl md:text-3xl mt-1" />
            <div className="flex flex-col max-md:items-center">
              <h5 className="text-xs md:text-sm">{skill.title}</h5>
              <span className="text-muted text-xs md:text-sm">{skill.level}</span>
            </div>
          </div>
        );
      });
